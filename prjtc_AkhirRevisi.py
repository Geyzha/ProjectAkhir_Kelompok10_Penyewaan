...
def validasi_nomor_telepon(prompt):
    """Validasi no telepon: harus angka, diawali '08', panjang 10-13."""
    while True:
        no_telp = input(prompt).strip()
        if not no_telp.isdigit():
            print("✗ No telepon harus berupa angka saja!")
            continue
        if not no_telp.startswith("08"):
            print("✗ No telepon harus diawali dengan '08'!")
            continue
        if not (10 <= len(no_telp) <= 13):
            print("✗ Panjang no telepon harus antara 10 sampai 13 digit!")
            continue
        return no_telp

def validasi_jaminan(jenis_jaminan):
    """Validasi nomor jaminan: KTP harus 16 digit angka; SIM harus 16 karakter alfanumerik."""
    while True:
        no_jaminan = input(f"Nomor {jenis_jaminan}: ").strip()
        if not no_jaminan:
            print(f"✗ Nomor {jenis_jaminan} tidak boleh kosong!")
            continue
        if jenis_jaminan == "KTP":
            if no_jaminan.isdigit() and len(no_jaminan) == 16:
                return no_jaminan
            else:
                print("✗ KTP harus 16 digit angka!")
        else:  # SIM
            if no_jaminan.isalnum() and len(no_jaminan) == 16:
                return no_jaminan
            else:
                print("✗ SIM harus 16 karakter alfanumerik!")
...
def sewa_alat():
    ...
    no_telp = validasi_nomor_telepon("Nomor telepon (harus diawali 08, 10-13 digit): ")
    while True:
        print("\nJenis Jaminan:")
        print("1. KTP")
        print("2. SIM")
    ...
    no_jaminan = validasi_jaminan(jenis_jaminan)
    while True:
    ...
    daftar_penyewa.append({
        ...
        # jaminan ditahan atau tidak (awal: ditahan karena jaminan diambil)
        'jaminan_ditahan': True if (sisa_tagihan > 0) else False
    })

# edit data penyewa
def edit_data_penyewa():
    print("\n=== Edit Data Penyewa ===")
    penyewa_aktif = [p for p in daftar_penyewa if p['status'] == 'Disewa']
    if not penyewa_aktif:
        print("Tidak ada penyewaan aktif untuk diedit.")
        return
    for i, data in enumerate(penyewa_aktif, 1):
        print(f"{i}. {data['nama']} - {data['jenis']} {data['tipe']} {data['merek']} - {data['no_telp']}")
    while True:
        try:
            pilihan = int(input("\nPilih nomor penyewa yang ingin diedit: "))
    ...
    print("\nData saat ini:")
    print(f"1. Nama         : {data['nama']}")
    print(f"2. No. Telepon  : {data['no_telp']}")
    print(f"3. Jaminan      : {data['jenis_jaminan']} ({data['no_jaminan']})")
    print(f"4. Alat         : {data['jenis']} - {data['tipe']} - {data['merek']}")
    print(f"5. Lama sewa    : {data['lama_hari']} hari")
    print(f"6. Kondisi awal : {data['kondisi_sewa']}")
    print("0. Selesai / Batal")
    while True:
        pilih_edit = input("\nPilih nomor field yang ingin diedit (0 untuk selesai): ").strip()
        if pilih_edit == "0":
            print("Selesai mengedit.")
            break
        elif pilih_edit == "1":
            new_nama = validasi_input_alfanumerik("Nama baru: ").title()
            data['nama'] = new_nama
            print("✓ Nama diperbarui.")
        elif pilih_edit == "2":
            new_telp = validasi_nomor_telepon("No. telepon baru: ")
            data['no_telp'] = new_telp
            print("✓ No. telepon diperbarui.")
        elif pilih_edit == "3":
            print("Jenis jaminan saat ini:", data['jenis_jaminan'])
            while True:
                print("1. Ubah nomor jaminan (jenis sama)")
                print("2. Ubah jenis jaminan (KTP <-> SIM) dan nomor")
                print("0. Kembali")
                sub = input("Pilih: ").strip()
                if sub == "1":
                    new_no = validasi_jaminan(data['jenis_jaminan'])
                    data['no_jaminan'] = new_no
                    print("✓ Nomor jaminan diperbarui.")
                    break
                elif sub == "2":
                    while True:
                        jenis_new = input("Pilih jenis baru (KTP/SIM): ").upper().strip()
                        if jenis_new in ("KTP", "SIM"):
                            break
                        else:
                            print("✗ Masukkan 'KTP' atau 'SIM'.")
                    nomor_baru = validasi_jaminan(jenis_new)
                    data['jenis_jaminan'] = jenis_new
                    data['no_jaminan'] = nomor_baru
                    print("✓ Jenis & nomor jaminan diperbarui.")
                    break
                elif sub == "0":
                    break
                else:
            ...
            # Mengganti alat: kembalikan stok untuk alat lama, kurangi stok alat baru
            print("Pilih alat baru (alat lama akan dikembalikan ke stok).")
            tampilkan_alat()
            while True:
                jenis_baru = input("Nama alat baru: ").title().strip()
                if jenis_baru not in alat_pendakian:
                    print("✗ Alat tidak ditemukan!")
                    continue
                break
            print("Tipe tersedia:")
            for t in alat_pendakian[jenis_baru].keys():
                print("-", t)
            while True:
                tipe_baru = input("Tipe baru: ").title().strip()
                if tipe_baru not in alat_pendakian[jenis_baru]:
                    print("✗ Tipe tidak ditemukan!")
                    continue
                break
            print("Merek tersedia:")
            for m in alat_pendakian[jenis_baru][tipe_baru].keys():
                print("-", m)
            while True:
                merek_baru = input("Merek baru: ").title().strip()
                if merek_baru not in alat_pendakian[jenis_baru][tipe_baru]:
                    print("✗ Merek tidak ditemukan!")
                    continue
                if alat_pendakian[jenis_baru][tipe_baru][merek_baru]['stok'] <= 0:
                    print("✗ Stok habis!")
                    continue
                break
            ...
            alat_pendakian[jenis_baru][tipe_baru][merek_baru]['stok'] -= 1
            # update data
            data['jenis'] = jenis_baru
            data['tipe'] = tipe_baru
            data['merek'] = merek_baru
            # update harga per hari (jika ingin mengikuti merek baru)
            data['harga_per_hari'] = alat_pendakian[jenis_baru][tipe_baru][merek_baru]['harga']
            print("✓ Alat penyewaan diperbarui.")
        elif pilih_edit == "5":
            while True:
                try:
                    lama_baru = int(input("Lama sewa baru (hari, 1-5): "))
                    if 1 <= lama_baru <= 5:
                        data['lama_hari'] = lama_baru
                        data['tanggal_kembali'] = data['tanggal_sewa'] + datetime.timedelta(days=lama_baru)
                        # update total biaya berdasarkan harga_per_hari
                        hargaph = data.get('harga_per_hari', data.get('total_biaya',0)//data.get('lama_hari',1))
                        data['total_biaya'] = hargaph * lama_baru
                        print("✓ Lama sewa diperbarui.")
                        break
                    else:
                        print("✗ Lama sewa harus 1-5 hari!")
                except ValueError:
            ...
            while True:
                print("Kondisi saat disewa saat ini:", data['kondisi_sewa'])
                print("1. Bagus")
                print("2. Cacat")
                print("0. Batal")
                kk = input("Pilih: ").strip()
                if kk == "1":
                    data['kondisi_sewa'] = "Bagus"
                    print("✓ Kondisi awal diperbarui ke 'Bagus'.")
                    break
                elif kk == "2":
                    data['kondisi_sewa'] = "Cacat"
                    print("✓ Kondisi awal diperbarui ke 'Cacat'.")
                    break
                elif kk == "0":
                    break
                else:
...
def batalkan_transaksi():
    ...
    penyewa_aktif = [p for p in daftar_penyewa if p['status'] == 'Disewa']
    if not penyewa_aktif:
        print("Tidak ada penyewaan aktif untuk dibatalkan.")
        return
    for i, data in enumerate(penyewa_aktif, 1):
        print(f"{i}. {data['nama']} - {data['jenis']} {data['tipe']} {data['merek']} - {data['no_telp']}")
    while True:
        try:
            pilihan = int(input("\nPilih nomor penyewaan yang ingin dibatalkan: "))
            if 1 <= pilihan <= len(penyewa_aktif):
                break
            else:
                print(f"✗ Nomor tidak valid! Masukkan angka 1-{len(penyewa_aktif)}.")
            ...
            print("✗ Input tidak valid! Masukkan angka.")
    data = penyewa_aktif[pilihan - 1]
    konfirmasi = input(f"Yakin ingin membatalkan transaksi penyewaan {data['nama']}? (y/n): ").lower().strip()
    if konfirmasi != 'y':
    ...
    try:
        ...
        pass
    # refund uang yang sudah dibayar (jika ada)
    refund = data.get('uang_dibayar', 0)
    # hapus dari daftar_penyewa
    try:
        daftar_penyewa.remove(data)
    except ValueError:
        # jika ada multiple references, hapus by identity
        for idx, d in enumerate(daftar_penyewa):
            if d is data:
                del daftar_penyewa[idx]
                break
    print(f"✓ Transaksi dibatalkan. Jumlah yang dikembalikan: {format_rupiah(refund)} (jika ada). Stok telah direstorasi.")
...
def kembalikan_alat():
    ...
    # Jaminan: ditahan jika masih ada sisa tagihan atau ada denda kondisi > 0
    if sisa_tagihan_setelah > 0 or denda_kondisi > 0:
        data['jaminan_ditahan'] = True
        print(f"\n⚠ Jaminan {data['jenis_jaminan']} ({data['no_jaminan']}) DITAHAN sampai pelunasan. (Pembayaran belum lunas)")
    else:
        data['jaminan_ditahan'] = False
    ...
    riwayat_pengembalian.append({
        ...
        'jaminan_dikembalikan': not data['jaminan_ditahan']
    })
...
# laporan (hampir sama, ditambahkan info jaminan)
def laporan_barang_disewa():
    print("\n" + "=" * 160)
    print("LAPORAN BARANG SEDANG DISEWA".center(160))
    print("=" * 160)
    penyewa_aktif = [p for p in daftar_penyewa if p['status'] == 'Disewa']
    if not penyewa_aktif:
        print("Tidak ada barang yang sedang disewa.".center(160))
        print("=" * 160)
        return
    print(f"{'No':<5} {'Alat':<30} {'Merek':<12} {'Kondisi':<10} {'Penyewa':<18} {'No. Telp':<15} {'Jaminan':<20} {'Tgl Sewa':<12} {'Tgl Kembali':<12} {'Pembayaran':<20} {'Biaya':<15}")
    print_line(length=160)
    for i, data in enumerate(penyewa_aktif, 1):
        ...
        jaminan = f"{data['jenis_jaminan']} ({'DITAHAN' if data.get('jaminan_ditahan',False) else 'ADA'})"
        ...
        if pembayaran == 'DP':
            ...
            pembayaran = "Lunas"
        print(f"{i:<5} {alat:<30} {merek:<12} {kondisi:<10} {data['nama']:<18} {data['no_telp']:<15} {jaminan:<20} {tgl_sewa:<12} {tgl_kembali:<12} {pembayaran:<20} {biaya:<15}")
    print("=" * 160)
...
def laporan_barang_dikembalikan():
    ...
    if not riwayat_pengembalian:
        ...
        print("=" * 200)
        return
    print(f"{'No':<4} {'Alat':<25} {'Merek':<12} {'Penyewa':<18} {'No.Telp':<13} {'Kondisi Sewa':<14} {'Kembali':<10} {'Pembayaran':<20} {'Tgl Sewa':<11} {'Tgl Kembali':<11} {'Denda Telat':<13} {'Denda Kondisi':<15} {'Total Bayar':<13} {'Jaminan Kembali':<18}")
    ...
    for i, data in enumerate(riwayat_pengembalian, 1):
        ...
        pembayaran = data.get('pembayaran_status', 'Belum Bayar')
        sisa = data.get('sisa_tagihan', 0)
        jaminan_kembali = "YA" if data.get('jaminan_dikembalikan', False) else "TIDAK (DITAHAN)"
        if isinstance(pembayaran, str) and pembayaran.startswith('DP') and sisa > 0:
            pembayaran = f"DP (sisa {format_rupiah(sisa)})"
        print(f"{i:<4} {alat:<25} {merek:<12} {data['nama']:<18} {data['no_telp']:<13} {kondisi_sewa:<14} {kondisi_kembali:<10} {pembayaran:<20} {tgl_sewa:<11} {tgl_kembali:<11} {denda_telat:<13} {denda_kondisi:<15} {total:<13} {jaminan_kembali:<18}")
    print("=" * 200)
...
def cari_penyewa():
    ...
    for i, data in enumerate(hasil, 1):
        ...
        print(f"   Jaminan         : {data['jenis_jaminan']} ({'DITAHAN' if data.get('jaminan_ditahan',False) else data.get('no_jaminan','-')})")
        ...
        if data['status'] == "Dikembalikan":
            ...
            print(f"   Jaminan Kembali : {'TIDAK (DITAHAN)' if data.get('jaminan_ditahan',False) else 'YA'}")
        else:
...
def menu_kelola_penyewa():
    while True:
        print("\n===== KELOLA PENYEWA =====")
        print("1. Edit Data Penyewa")
        ...
        if pilihan == "1":
            ...
            print("✗ Pilihan tidak valid! Masukkan angka 0-2.")
        pause()
...
def menu():
    while True:
        ...
        print("5. Laporan")
        print("6. Kelola Penyewa (Edit / Batalkan)")
        print("0. Keluar")
        print("=" * 40)
        pilihan = input("\nPilih menu (0-6): ").strip()
        if pilihan == "1":
